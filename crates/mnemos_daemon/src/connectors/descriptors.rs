...
fn claude_mcp_path() -> PathBuf { home().join(".claude.json") }                                 // JSON, mcpServers
fn claude_md_path() -> PathBuf { home().join(".claude").join("CLAUDE.md") }
fn codex_config_path() -> PathBuf { home().join(".codex").join("config.toml") }                 // TOML, [mcp_servers.*]
fn codex_agents_path() -> PathBuf { home().join(".codex").join("AGENTS.md") }
fn gemini_settings_path() -> PathBuf { home().join(".gemini").join("settings.json") }           // JSON, mcpServers
fn antigravity_dir() -> PathBuf { home().join(".gemini").join("antigravity-cli") }
fn antigravity_mcp_path() -> PathBuf { antigravity_dir().join("mcp_config.json") }               // JSON, mcpServers
    ...
    vec![
        ToolConnector {
            ...
            edits: vec![
                ...
                ConfigEdit { target: claude_md_path, strategy: EditStrategy::MarkedBlock { body: HINT } },
            ],
            manual_snippet: None,
        },
        ToolConnector {
            ...
            edits: vec![
                ...
                ConfigEdit { target: codex_agents_path, strategy: EditStrategy::MarkedBlock { body: HINT } },
            ],
            manual_snippet: None,
        },
        ToolConnector {
...
mod tests {
    ...
    fn registry_has_expected_tools_kinds_and_strategies() {
        ...
        assert!(matches!(codex.edits[0].strategy, EditStrategy::TomlMerge { .. }), "Codex uses TOML");
        assert!(r.iter().find(|c| c.id == "gemini-cli").unwrap().deprecated.is_some());
